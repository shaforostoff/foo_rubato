...
`bpmcore/rhythm_model.h` and walked directly. Leaf values are stored as
`double`. Thresholds are stored as `float`, which is not a rounding: sklearn
bins `float32` features, so all 10,500 thresholds are already `float` values
widened to `double`, and `classify` compares a `double` feature against one,
which widens it straight back to the number the fit produced. The generator
checks that instead of assuming it, and refuses to narrow a threshold that a
refit has moved off the `float` grid — half an ulp there is enough to send a
feature down the other side of a split, which reads as a slightly different
answer on a handful of tracks rather than as a failure.
...
522,000, and the x64 DLL 694KB instead of 959KB. Narrowing the thresholds took
another 42,000 bytes off every binary afterwards, 198,000 in all.
...
ones repeat enough that deflate had already found most of it. Leaf values are
the one part of the model that does not compress — 11,250 doubles, all
distinct, 90,000 bytes that deflate to 86,078.
